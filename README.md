...
- Tailwind CSS 3.4 (цвета завязаны на CSS-переменные Telegram)
...
Откроется на http://localhost:5173/. Приложение работает и в обычном браузере (используются дефолты светлой/тёмной темы через `prefers-color-scheme`).
...
SDK вызывается в `src/lib/telegram.ts`: при запуске внутри Telegram вызываются `ready()`, `expand()`, `disableVerticalSwipes()`; подписка на `themeChanged` применяет актуальные цвета. В обычном браузере вызовы no-op, дефолты берутся из `src/index.css`.
...
├── index.css                   # Tailwind + :root дефолты tg-переменных
...
- **Тема:** Tailwind-цвета (`bg-tg-bg`, `text-tg-text` и т. д.) ссылаются на `var(--tg-theme-*-color)`. Дефолты в `src/index.css :root` для светлой и тёмной темы, переопределяются `themeParams` из Telegram при запуске внутри клиента.
